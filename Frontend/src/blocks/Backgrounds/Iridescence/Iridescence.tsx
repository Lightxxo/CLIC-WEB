...
void main() {
  ...
  vec2 uv = vec2(
    (vUv.x * 2.0 - 1.0) * (uResolution.x / uResolution.y),
    vUv.y * 2.0 - 1.0
  );

  uv += (uMouse - vec2(0.5)) * uAmplitude;

  float d = -uTime * 0.5 * uSpeed + uScroll * 2.0;
  float a = 0.0;
  for (float i = 0.0; i < 8.0; ++i) {
    a += cos(i - d - a * uv.x);
    d += sin(uv.y * i + a);
  }
  d += uTime * 0.5 * uSpeed + uScroll * 2.0;
...
export default function Iridescence({
  ...
  useEffect(() => {
    ...
    function update(t: number) {
      ...
      program.uniforms.uScroll.value = scrollPos.current * 0.001;
    ...
    function handleScroll() {
      scrollPos.current = window.scrollY || window.pageYOffset || 0;
    }
